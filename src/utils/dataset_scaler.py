...
class DatasetScaler:
    ...
    def scale_hdf5(
        ...
        *,
        ...
        with h5py.File(str(input_path), "r") as src, h5py.File(
            ...
            neighbor_steps = math.ceil(neighbor_ds.shape[0] / neighbor_chunk)

            total_steps = scale_factor + 1 + neighbor_steps
            self._emit_progress(0, total_steps)

            self._ensure_parent_group(dst, base_dataset)
            ...
            for factor_index in range(scale_factor):
                ...
                completed_steps += 1
                self._emit_progress(completed_steps, total_steps)
            ...
            self._ensure_parent_group(dst, neighbor_dataset)
    ...
    @staticmethod
    def _ensure_parent_group(file: h5py.File, path: str) -> None:
        """Ensure the parent group exists for a dataset path."""
    ...
    def _create_dataset(
        ...
        DatasetScaler._ensure_parent_group(file, path)
    ...
    def _copy_dataset(
        ...
        DatasetScaler._ensure_parent_group(destination_file, path)
